...
new Elysia()
  ...
  .get("/", async ({ query, set, redirect, headers, cookie }) => {
    ...
    if (!rateCheck.allowed) {
      ...
      return redirect(`/c/${encodeURIComponent(await signRedirect(currentUrl))}`);
    }
  ...
  .get("/p/:q", async ({ set, params, cookie, redirect, headers }) => {
    ...
    if (!rateCheck.allowed) {
      return redirect(
        `/c/${encodeURIComponent(await signRedirect(`/a/${params.q}`))}`,
      );
    }
